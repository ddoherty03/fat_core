...
- [[#usage][Usage]]
    ...
    - [[#enumerable][Enumerable]]
      ...
      - [[#method-groups_ofn][Method ~#groups_of(n)~]]
      - [[#method-each_group_ofn][Method ~#each_group_of(n)~]]
    - [[#hash][Hash]]
    ...
    - [[#string][String]]
      ...
      - [[#method-commansplaces][Method ~#commans(places)~]]
    - [[#symbol][Symbol]]
...
**** Method ~#groups_of(n)~
This method returns the elements of the ~Enumerable~ in arrays of size ~n~.
The last group will not be of full size if the size of the ~Enumerable~ is not
evenly divisible ~n~.

#+begin_src ruby :ruby ruby
  require_relative 'lib/fat_core/enumerable'

  result = []
  abcs = ('A'..'Z').to_a
  abcs.groups_of(4).each do |g|
    result << g
  end
  result
#+end_src
...
**** Method ~#each_group_of(n)~
This method yields each group returned by the ~groups_by~ method to the given
block.
...
|        a..z | ["a".."g", "h".."s", "t".."z"]      | true        | []                    | []       |
|        a..z | ["a".."g", "j".."s", "t".."z"]      | false       | ["h".."i", "aa".."z"] | []       |
#+end_example
